...
const ACTION={
  toggleTheme:'moon',screenBreak:'pause',restBreak:'bed',goFocus:'calendar-lines',coffee:'mug-hot',
  startWork:'play',endWork:'stop',endBreak:'check',extendBreak:'plus',
  startActivity:'play',pauseActivity:'pause',completeActivity:'check',editActivity:'edit',cancelActivity:'trash',duplicateActivity:'copy-alt',moveTomorrow:'arrow-right',
...
function semanticFromContext(svg){
  ...
  if(svg.closest('[data-plan-new]'))return'plus';
  if(svg.closest('[data-plan-activities]'))return'list-check';
  if(svg.closest('[data-plan-shift]'))return'calendar';
  if(svg.closest('[data-plan-history]'))return'time-past';
  if(svg.closest('[data-plan-today]'))return'home';
  if(svg.closest('.notification-bell'))return'bell';
  if(svg.closest('.hub-close,.sp-close,.transport-close,.hub-about-close,.fj-help-close,.professional-close'))return'cross';
  if(svg.closest('.hub-row-arrow,.planning-arrow'))return'angle-small-right';
  if(svg.closest('.hub-future-icon,.transport-launch-icon'))return'apps';
  if(svg.closest('.forecast-icon'))return'clock';
  if(svg.closest('#jumpMoovit'))return'bus';
  if(svg.closest('#jumpSupershift'))return'calendar';
  if(svg.closest('.manual-chip'))return'bell';
  if(svg.closest('.sp-add'))return'plus';
  if(svg.closest('.sp-checkbox'))return'check';
  if(svg.closest('.logo'))return'badge-check';
...
function semanticFromPath(svg){
  ...
  const tests=[
    ['M3 10.5 12 3','home'],['x="3" y="4" width="18" height="16"','list-check'],['M3 12a9 9','time-past'],
    ...
    ['m15 18-6-6','angle-left'],['M12 3v12M8 7','share'],['M8 4v16','sort-alt'],['M14 4h6v6','external-link'],['M3 11 12 3l9 8','home'],
    ['M5 17h14l1-8','bus'],['x="3" y="5" width="18" height="16"','calendar'],['M12 21s6-5.2','marker'],['M8 3v4M16 3v4','apps']
  ];
...
function replaceTextIcons(scope=document){
  scope.querySelectorAll?.('.sp-job-icon').forEach(el=>{if(el.dataset.flaticonText)return;el.dataset.flaticonText='1';const i=document.createElement('i');i.className='fi fi-rr-briefcase fi-motion fi-role-briefcase flaticon-live';i.setAttribute('aria-hidden','true');el.textContent='';el.appendChild(i)});
}
function enhance(scope=document){
  if(!ready)return;
  scope.querySelectorAll?.('svg.ui-svg,svg.hub-svg,svg.sp-icon,svg.feature-svg,#professionalCommandCenter svg,#professionalSearch svg,#professionalDiagnostics svg,#hubAbout svg,#fjHelp svg').forEach(bindSvg);
...
new MutationObserver(mutations=>{if(!ready)return;for(const m of mutations)for(const n of m.addedNodes)if(n.nodeType===1){schedule(document);return}}).observe(document.body,{childList:true,subtree:true});

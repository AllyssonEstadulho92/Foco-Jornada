const FLATICON_VERSION='3.1.0';
const CDN_MARK='cdn-uicons.flaticon.com';
let ready=false,raf=0;

const NAV={today:'home',activities:'checkbox',planning:'apps',history:'time-past',stats:'chart-histogram',more:'menu-dots'};
const ACTION={
  toggleTheme:'moon',screenBreak:'pause',restBreak:'bed',goFocus:'apps',coffee:'mug-hot',
...
const HUB={moovit:'bus',supershift:'calendar',shifts:'calendar',schedule:'clock',planning:'apps',stats:'chart-histogram',settings:'settings',backup:'database',notifications:'bell',updates:'refresh',about:'info'};
...
function semanticFromPath(svg){
  ...
  const tests=[
    ...
    ['M20 7v5','refresh'],['M12 7v5l3 2','clock'],['M12 11v6','info'],['m9 6 6 6','angle-small-right'],['x="4" y="4" width="6"','apps'],['x="3" y="3" width="7"','apps'],
...
function enhance(scope=document){if(!ready)return;scope.querySelectorAll?.('svg.ui-svg,svg.hub-svg,svg.sp-icon').forEach(bindSvg);replaceTextIcons(scope)}
function schedule(scope=document){if(!ready||raf)return;raf=requestAnimationFrame(()=>{raf=0;enhance(scope)})}
...
if(stylesheetPresent())activate();else if(link){link.addEventListener('load',activate,{once:true});link.addEventListener('error',()=>document.documentElement.classList.add('flaticon-fallback'),{once:true})}

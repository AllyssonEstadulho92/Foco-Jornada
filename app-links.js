...
function launchMoovitScheme(url,status='A abrir no Moovit…'){
  ...
  fallbackTimer=setTimeout(()=>{
    if(!leftPage&&document.visibilityState==='visible')window.location.href=MOOVIT_FALLBACK;
  },1500);
}
...
function bindPlan(){
  let btn=ensureButton($('#planMoovit'),'planMoovit');if(!btn||btn.dataset.moovitBound==='1')return;
  btn.dataset.moovitBound='1';
...
function bindNearby(){
  let btn=ensureButton($('#nearbyTransit'),'nearbyTransit');if(!btn||btn.dataset.moovitBound==='1')return;
  btn.dataset.moovitBound='1';
  btn.addEventListener('click',e=>{
    e.preventDefault();e.stopImmediatePropagation();
    const url=buildMoovitNearbyUrl({partnerId:MOOVIT_PARTNER_ID});
    launchMoovitScheme(url,'A abrir linhas e paragens perto de ti…');
  },true);
}
function bindQuickTrips(){
  document.querySelectorAll('#moovitSection [data-trip]').forEach(btn=>{
    if(btn.dataset.moovitBound==='1')return;btn.dataset.moovitBound='1';
    btn.addEventListener('click',e=>{
      e.preventDefault();e.stopImmediatePropagation();
      const type=btn.dataset.trip,origin=type==='home'?'work':'current',dest=type==='home'?'home':'work',f=readFeature();
      if(!validateRoute(origin,dest,f))return;
...
function enhance(){ensureStyles();bindPlan();bindNearby();bindQuickTrips();replaceSupershift()}
...
window.FocoAppLinks=Object.freeze({version:'2.1.0',enhance,launchMoovit:launchMoovitScheme});

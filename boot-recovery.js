...
function fallbackNavigate(e){
  ...
  const title=$('#pageTitle');if(title)title.textContent=({today:'Hoje',activities:'Atividades',focus:'Foco',history:'Histórico',stats:'Estatísticas',more:'Mais'})[view]||'Foco & Jornada';
}
...
function showRecoveryNotice(){
  ...
  notice.style.cssText='position:fixed;left:12px;right:12px;bottom:calc(82px + env(safe-area-inset-bottom));z-index:9999;display:flex;align-items:center;justify-content:space-between;gap:10px;padding:12px 14px;border:1px solid rgba(255,255,255,.16);border-radius:16px;background:#111b28;color:#fff;box-shadow:0 16px 44px #0008;font:600 13px/1.35 system-ui,-apple-system,sans-serif';
  notice.innerHTML='<span>A aplicação não terminou o arranque.</span><button type="button" style="border:0;border-radius:11px;padding:9px 12px;background:#2388ff;color:white;font:700 13px system-ui,-apple-system,sans-serif">Recarregar</button>';

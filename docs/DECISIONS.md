...
## D-044 — Sinalizar consulta parcial sem modificar o saldo ou as fontes

**Estado:** proposta no PR #224, não integrada nem publicada. **Evidência:** o painel de proveniência não invalidava o `useMemo` no regresso à aplicação e o coletor ocultava erros de leitura/JSON/estrutura das chaves mensais. **Decisão:** acrescentar argumento opcional de diagnóstico em `collectVacationEvidenceForYear` para descrever apenas origem, mês e categoria do erro; conservar a lista de datas e o contrato de `collectVacationDatesForYear`. Quando há erros detetáveis, indicar no painel que a consulta e totais podem estar incompletos. Atualizar a revisão ao receber foco, visibilidade ou sinal de gravação do cofre. **Motivo:** evitar confiança injustificada em informação parcial mantendo privacidade e compatibilidade. **Limites:** `null` não distingue ausência de dados e cofre não vinculado, um evento local não comprova sincronização remota, e `VacationBalancePage` mantém coletor próprio; investigar antes de unificar e não corrigir cálculos sem provas. **Segurança e reversibilidade:** nenhuma escrita, novo armazenamento, API, token, permissão, esquema ou regra laboral; alterações isoladas e testes de casos limite. CI e ensaio físico pendentes. Ver `docs/PROJECT_STATE.md` e [PR #224](https://github.com/AllyssonEstadulho92/Foco-Jornada/pull/224).

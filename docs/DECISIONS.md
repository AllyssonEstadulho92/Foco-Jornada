...
## D-041 — Reduzir competição visual e aproveitar largura real sem alterar a estrutura de dados

**Estado:** proposto no PR #222; aguardar CI final, integração e publicação. **Decisão:** reutilizar `vacation-insights.css` para tornar os cartões de detalhe mais contidos e retirar apenas linhas decorativas repetidas, conservando destaque do progresso/«Próximo marco» e contraste de saldo negativo. Em `vacation-planning-structure.css`, apresentar `vacationJointChoice` e `vacationJointCompare` em colunas 4/12 + 8/12 apenas com largura >=1100px; manter DOM e disposição sequencial no móvel. O resumo opcional do ano atual distribui texto, saldo e sinal de expansão em grelha sem sobreposição, com duas linhas <=440px. **Motivo:** a captura fornecida e inspeção do CSS revelam densidade alta de cartões e demasiada altura no desktop; um mesmo sistema de componentes adapta-se a larguras distintas. **Risco/limite:** testes de CSS/CI não medem overflow real em iPhone, tipografia ampliada ou usabilidade assistiva; exigir captura e navegação físicas. **Preservação:** sem novo CSS global, sem mudança de fórmulas, registos, regras laborais, confirmações, autenticação, cofre, Worker, backend ou sincronização.

## D-040 — Propostas antes de contexto antigo e planeamento progressivo

**Estado:** aceite, PR #221 integrado/publicado. **Decisão:** no planeamento do próximo ano, apresentar primeiro as propostas e remeter a referência do saldo do ano corrente para `<details>` consultável, explicitando que não é saldo transferido. No ano corrente o resumo vivo mantém-se aberto. Reorganizar a proposta a dois em quatro etapas semânticas na ordem do DOM (preferências, comparação, simulação e confirmação), com opções compactas e notas detalhadas em `<details>`. **Motivo:** o PDF do utilizador mostrou quatro cartões de 2026 e alternativas altas antes da decisão sobre julho/2027; a melhoria reduz ruído e distingue horizonte atual de projeção futura. **Trade-off:** a folha de layout `vacation-planning-structure.css` escopada à rota suporta os grupos reais sem reformular a cascata global; os detalhes nativos mantêm informação secundária consultável. **Preservação:** mesmas funções de domínio, cofre, dados, alternativas, bloqueio declarado de novembro/dezembro, simulação e checklist efémera; nenhuma aprovação externa. **Entrega:** merge `9e9d267fd7a9ed7de3a9afdc68cbc8a8d2b708b9`, Qualidade #1227/#1228, Publicar #260, build `4074f203c6788d30a4f542c951a6b94b62f86fad`, Pages #866; teste físico pendente. Ver `docs/VACATION-PLANNING-STRUCTURE-2026.md`.

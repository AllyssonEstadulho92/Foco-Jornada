...
- O PDF de planeamento anterior mostrou o ecrã de 2027 com quatro cartões do saldo de 2026 antes das propostas e lista extensa de alternativas. Confirmada a ordem no código, sem erro de cálculo demonstrado.
...
- `vacation-planning-structure.css` é exclusivo da rota, com responsividade, foco, alto contraste e movimento reduzido. Testes e `VACATION-PLANNING-STRUCTURE-2026.md`.
...
- `vacation-visual-audit.css` recebeu largura de leitura de 74rem, superfícies/sombras discretas, hierarquia de saldo e calendário em sete colunas contidas. Sem esconder funções nem alterar valores.
...
- `#/ferias` recebeu painel opcional «De onde vêm os teus dias?», datas/fontes deduplicadas, úteis gozados/futuros, fins de semana ignorados e manuais sem data. `VacationYearRecords` expõe proveniência única e valida datas, com leitura do cofre existente e testes de domínio/UI.

...
function dataSummary(){const s=readJSON(APP_KEY,{}),f=readJSON(FEATURE_KEY,{});return{works:s.workSessions?.length||0,activities:s.activities?.length||0,coffee:s.coffeeEntries?.length||0,shifts:Object.keys(f.shiftPlanner?.assignments||{}).length}}
...
function buildDiagnostic(){const d=dataSummary();return[`Foco & Jornada ${VERSION}`,`Modo: ${standalone()?'PWA instalada':'Browser'}`,`Rede: ${navigator.onLine?'Online':'Offline'}`,`Notificações: ${notificationState()}`,`Armazenamento local: ${fmtBytes(storageBytes())}`,`Jornadas: ${d.works}`,`Atividades: ${d.activities}`,`Cafés: ${d.coffee}`,`Dias com turno: ${d.shifts}`].join('\n')}
...
function openAbout(){
  ...
  serviceWorkerState().then(v=>{const el=$('#aboutSwState');if(el)el.textContent=v});window.FocoFlaticon?.refresh?.();requestAnimationFrame(()=>p.classList.add('open'));
}

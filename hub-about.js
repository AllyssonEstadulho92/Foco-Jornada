...
function dataSummary(){const s=readJSON(APP_KEY,{}),f=readJSON(FEATURE_KEY,{});return{works:s.workSessions?.length||0,activities:s.activities?.length||0,legacySessions:s.focusSessions?.length||0,coffee:s.coffeeEntries?.length||0,shifts:Object.keys(f.shiftPlanner?.assignments||{}).length}}
...
function buildDiagnostic(){const d=dataSummary();return[`Foco & Jornada ${VERSION}`,`Modo: ${standalone()?'PWA instalada':'Browser'}`,`Rede: ${navigator.onLine?'Online':'Offline'}`,`Notificações: ${notificationState()}`,`Armazenamento local: ${fmtBytes(storageBytes())}`,`Jornadas: ${d.works}`,`Atividades: ${d.activities}`,`Registos antigos: ${d.legacySessions}`,`Cafés: ${d.coffee}`,`Dias com turno: ${d.shifts}`].join('\n')}
...
function openAbout(){
  ...
  p.innerHTML=`<div class="hub-about-head"><div><span class="kicker">INFORMAÇÃO</span><h3>Sobre</h3></div><button type="button" class="hub-about-close" data-about-close aria-label="Fechar">${fi('cross')}</button></div><div class="hub-about-card hub-about-brand"><strong>Foco & Jornada</strong><p>Aplicação local-first para jornada de trabalho, pausas, atividades, planeamento, transportes, escala e histórico.</p></div><section class="hub-about-section"><h4>Aplicação</h4><div class="hub-about-card">${row('Versão',VERSION)}${row('Instalação',standalone()?'PWA instalada':'Aberta no browser')}${row('Estado da rede',navigator.onLine?'Online':'Offline')}${row('Service Worker','A verificar…','aboutSwState')}${row('Notificações',notificationState())}${row('Armazenamento',fmtBytes(storageBytes()))}</div></section><section class="hub-about-section"><h4>Dados neste dispositivo</h4><div class="hub-about-card">${row('Jornadas',d.works)}${row('Atividades',d.activities)}${d.legacySessions?row('Registos antigos',d.legacySessions):''}${row('Registos de café',d.coffee)}${row('Dias com turno',d.shifts)}</div></section><section class="hub-about-section"><h4>Módulos</h4><div class="hub-about-modules"><span>Jornada</span><span>Atividades</span><span>Planeamento</span><span>Histórico</span><span>Estatísticas</span><span>Moovit</span><span>Supershift</span><span>Backup</span><span>Notificações</span></div></section><div class="hub-about-note">Os dados principais são guardados localmente neste dispositivo e têm uma camada de recuperação local. A aplicação não tem atualmente conta de utilizador, base de dados cloud ou sincronização automática entre dispositivos.</div><div class="hub-about-note flaticon-attribution">Ícones de interface por <a href="https://www.flaticon.com/uicons" target="_blank" rel="noopener noreferrer">Flaticon UIcons</a>.</div><div class="hub-about-actions"><button type="button" class="btn" id="aboutCopyDiagnostic">Copiar diagnóstico</button><button type="button" class="btn" id="aboutCheckUpdate">Verificar atualização</button></div>`;

...
test('Sobre não mantém crédito de uma camada de ícones já removida',()=>{
  const about=read('hub-about.js');
  assert.equal(about.includes('Flaticon'),false);
...
test('publicação mantém Sobre mas já não envia deduplicador legado',()=>{
  const sw=read('sw.js'),pages=read('.github/workflows/pages.yml'),couple=read('couple.js');
  for(const f of ['hub-about.js','hub-about.css']){assert.ok(sw.includes(f),`cache ${f}`);assert.ok(pages.includes(f),`pages ${f}`)}

...
test('menu superior móvel existe mas permanece oculto por CSS',()=>{assert.ok(js.includes('fjTopMore'));assert.ok(js.includes('window.FocoHub?.open?.()'));assert.ok(css.includes('.fj-more-trigger'));assert.ok(css.includes('display:none!important'));});

...
test('Pomodoro automático passa foco para pausa curta e volta ao foco',()=>{const at=new Date(2026,7,19,8).getTime(),s=base(at);s.settings.focusMinutes=1;s.settings.shortBreakMinutes=1;s.settings.longBreakMinutes=2;s.settings.focusCycles=4;P.setPomodoroSettings(s,{auto:true,dailyGoal:6},at);const start=C.startFocus(s,at,{durationMinutes:1});const first=P.reconcilePomodoro(s,start.value.expectedEndAt);assert.equal(first.next.phase,'SHORT_BREAK');const second=P.reconcilePomodoro(s,first.next.expectedEndAt);assert.equal(second.next.phase,'FOCUS');assert.equal(P.focusDailyStats(s,second.next.startedAt).goal,6)});

test('quarto foco inicia pausa longa',()=>{const at=new Date(2026,7,19,8).getTime(),s=base(at);s.settings.focusMinutes=1;s.settings.shortBreakMinutes=1;s.settings.longBreakMinutes=1;s.settings.focusCycles=4;P.setPomodoroSettings(s,{auto:true},at);let t=at,last=null;for(let i=0;i<7;i++){if(!C.activeFocus(s))C.startFocus(s,t,{durationMinutes:1});const f=C.activeFocus(s);last=P.reconcilePomodoro(s,f.expectedEndAt);t=f.expectedEndAt}assert.equal(last.next.phase,'LONG_BREAK')});

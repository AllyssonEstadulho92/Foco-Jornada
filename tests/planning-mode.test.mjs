...
test('tipografia global usa apenas fontes locais e preserva a família dos ícones',()=>{assert.ok(linear.includes('font-family:ui-sans-serif,-apple-system,BlinkMacSystemFont'));assert.ok(linear.includes('!important'));assert.ok(linear.includes(':not(.fi)'));assert.equal(linear.includes('font-family:"Manrope"'),false);assert.equal(linear.includes('font-weight:650'),false);});

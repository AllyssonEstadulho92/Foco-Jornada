...
function useInstallIcon(){const touch=document.querySelector('link[rel="apple-touch-icon"]');if(touch)touch.href='./apple-touch-icon.png'}
...
async function waitStylesheet(id,timeout=2500){const link=document.getElementById(id);if(!link||link.sheet)return;await Promise.race([new Promise(resolve=>{link.addEventListener('load',resolve,{once:true});link.addEventListener('error',resolve,{once:true})}),sleep(timeout)])}
function revealApp(){root.dataset.fjBoot='ready';root.dataset.fjCoreReady='1';if(shell)shell.hidden=true}
function failBoot(error){console.error('[Foco & Jornada] Falha de arranque',error);root.dataset.fjBoot='error';setStatus('O arranque demorou demasiado. Recarrega para tentar novamente.');const retry=document.getElementById('startupRetry');if(retry){retry.hidden=false;retry.onclick=()=>location.reload()}}
...
async function loadExtras(){if(extrasPromise)return extrasPromise;extrasPromise=(async()=>{const modules=['./ux.js','./hub.js','./controls.js','./settings-controller.js','./app-links.js','./interaction-fixes.js','./runtime-fixes.js','./summary-guard.js','./professional-ui.js','./install-app.js'];for(const path of modules){try{await nextFrame();await sleep(45);await import(path)}catch(error){console.error(`[Foco & Jornada] Módulo opcional não carregado: ${path}`,error)}}})();return extrasPromise}
function scheduleExtras(){if(extrasScheduled)return;extrasScheduled=true;const start=()=>loadExtras().catch(error=>console.error('[Foco & Jornada] Extras não carregados',error));if('requestIdleCallback'in window){requestIdleCallback(start,{timeout:2200});return}setTimeout(start,900)}
...
async function bootCore(){setStatus('A preparar os dados locais…');await withTimeout(loadClassic('./persistence.js'),5000,'Persistência indisponível');await disableFirstUse();setStatus('A carregar a jornada…');await withTimeout(import('./stability.js'),8000,'O núcleo demorou demasiado');await waitStylesheet('mainCss')}

window.FocoBootstrap=Object.freeze({version:BOOT_VERSION,loadShift,loadExtras});

try{
  useInstallIcon();
  await withTimeout(bootCore(),12000,'Tempo limite de arranque excedido');
  revealApp();
  scheduleExtras();
